...
utilizado en proyectos similares con anterioridad, por ejemplo en juegos de mesa como el 
``Go''~\cite{b10} parece que esta aproximación es la que mejor funciona debido a la gran 
complejidad que posee el juego y que por tanto hace imposible la búsqueda de un camino
completamente óptimo como podría intentar una aproximación con el algoritmo MiniMax con
poda alfa-beta.
...
o no). El término disminuye cuanto mayor sea $n_j$ (y, por tanto, se haya explorado ese nodo), mientras
...
Organizar jerárquicamente, ordenar y completar
\item \textit{Creación de Agente modo Bot:}
\item \textit{Jugar contra bot:} 
\item \textit{Bot contra bot:} 
\item \textit{Grabacion de partidas a csv:} 
\item \textit{agente UCT:} 
\item \textit{¿¿poder cambiar quien controla negras y blancas indistintamente??} 
\item \textit{Simulación de X partidas:} 
\item \textit{Red neuronal} 
\item \textit{ETC (entrenar red, lo que vayamos haciendo)} 
