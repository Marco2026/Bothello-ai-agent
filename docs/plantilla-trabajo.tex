...
del juego~\cite{b3}-~\cite{b5} como del menú~\cite{b6}-~\cite{b7} mostrado en (Fig. \ref{fig:menuimage}.). Una vez realizado el
...
\begin{figure}
  \centering
  \includegraphics[width=0.7\columnwidth]{MENU_IMAGE.jpg}
  \caption{Imagen del menú en Pygame.}
  \label{fig:menuimage}
\end{figure}
...
A continuación presentamos una breve tabla de contenido sobre el presente documento:

\begin{enumerate}
  \item Introducción
  \item Preliminares
  \begin{enumerate}
    \item Métodos empleados
    \item Trabajo Relacionado
  \end{enumerate}
  \item Metodología
  \begin{enumerate}
    \item Preparación del proyecto
    \item Implementación de Otelo
    \item Habilitación de bot
    \item Creación del agente
    \item Grabación de partidas
    \item Simulación de partidas
    \item Creación de la red neuronal
  \end{enumerate}
  \item Resultados
  \begin{enumerate}
    \item Bot vs Bot
    \item Agente MCTS vs Bot
    \item Agente MCTS vs Agente MCTS
    \item Red Neuronal vs Bot
    \item Red Neuronal vs Agente MCTS
    \item Red Neuronal 1 vs Red Neuronal 2
  \end{enumerate}
  \item Conclusiones
  \item Uso de la IA generativa
  \item Bibliografía
\end{enumerate}
...
\item \textit{Creación del agente:} incorporamos un agente capaz de tomar decisiones
 estratégicas utilizando el algoritmo Monte Carlo Tree Search (MCTS) siguiendo el pseudocódigo
 explicado en (Fig. ~\ref{pcd:uct_pseudocodigo}.) basado en el explicado en ~\cite{b9}.
  \begin{enumerate}
...
\item \textit{Creación de la red neuronal:} Diseñamos una red neuronal capaz de predecir la probabilidad 
  ...
  Para la creación de la red hemos utilizado un optimizador $SGD$ y una estructura con:
  \begin{itemize}
    \item Capa inicial: 64 neuronas (una por cada posición del tablero)
    \item Capa intermedia: 32 neuronas y función de activación sigmoide
    \item Capa final: 1 neurona y función de activación de tangente hiperbólica (para obtener resultado en el intervalo $[-1,1]$)
  \end{itemize}

  Como hiperparámetros, hemos utilizado:

  \begin{equation}
  \label{eq:tasaaprendizaje_mse}
   \eta = 0,01 \qquad \text{MSE} = 1
  \end{equation}
  \begin{equation}
  \label{eq:epochs_bach}
   \text{epochs} = 800  \qquad \text{batch} = 256
  \end{equation}
...
\item \textit{Red Neuronal vs Agente MCTS}:
  ...
  \item \textit{Red Neuronal 1 vs Red Neuronal 2}:
  \begin{enumerate}
    \item \textit{Configuración}\\
    Para este experimento contamos con la siguiente configuración:
    \begin{itemize}
      \item \textit{Player 1}: Red Neuronal 1
      \item \textit{Player 2}: Red Neuronal 2
      \item \textit{Iteraciones para red 1}: $800$
      \item \textit{Iteraciones para red 2}: $1500$
    \end{itemize}
    \item \textit{Objetivo}\\
    Entrenamos a la red con más datos que la vez anterior (incluyendo las partidas que
    ella misma realizó contra el Bot y el Agente MCTS). Esperábamos que la red se comportara
    mejor que la vez anterior al haber recibido más datos y más iteraciones de entrenamiento.
    \item \textit{Resultados}\\
    Con una muestra de 100 partidas generadas, obtuvimos los siguientes resultados (Fig. ~\ref{fig:agentnn01vsagentnn02}.):
    \begin{itemize}
      \item \textit{Victorias de Player 1}: $42$
      \item \textit{Victorias de Player 2}: $41$
      \item \textit{Empates}: $7$
    \end{itemize}

    \begin{figure}
      \centering
      \includegraphics[width=\columnwidth]{AGENT_NN01vsAGENT_NN02.png}
      \caption{Resultados de Agente con Red Neuronal 1 contra Agente con Red Neuronal 2.}
      \label{fig:agentnn01vsagentnn02}
    \end{figure}
    Los resultados fueron algo decepcionantes, ya que tras entrenar de nuevo la red obtuvimos prácticamente
    el mismo resultado.
...
\begin{table}
  ...
  \begin{tabular}{ccccc}
    ...
    Agente NN01 & Agente NN02 & 42 & 7 & 41 \\
    \bottomrule
  \end{tabular}
\end{table}
