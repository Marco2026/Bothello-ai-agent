...
\begin{IEEEkeywords}
  ...
  Red Neuronal, Montecarlo Tree Search, Othello, Reversi, Python, Pygame.
\end{IEEEkeywords}
...
\item \textit{Grabación de partidas:} añadimos una funcionalidad para almacenar registros 
  de las partidas jugadas en un fichero csv, de modo que posteriormente se pueda utilizar
  para entrenar una red neuronal. Estos datos se pueden crear de manera opcional, en un 
  archivo \texttt{.csv} que se genera en la carpeta \texttt{agent/training}, y toma el
  ...
  Posteriormente, todos estos ficheros se copiarán pasarán a otro archivo que para entrenar la red.
...
\item \textit{Creación de la red neuronal:} Diseñamos una red neuronal capaz de predecir la probabilidad 
  ...
  simuladas, donde por cada turno se guardaba la disposición del tablero y si el resultado en la partida del 
  jugador protagonista en ese turno.
  \item \textit{Separación de distintas redes:} Una vez entrenamos la red, almacenamos los resultados del entrenamiento 
  ...
  con el fin de alcanzar un modelo mejorado que maximice la probabilidad de victoria del agente.
  \end{enumerate}
...
\item \textit{Bot vs Agente MCTS}:
  \begin{enumerate}
    ...
    \end{itemize}
    \item \textit{Objetivo}\\
    Dado que el Agente MCTS era tan superior al bot, pensamos que darle cierta
    ventaja al bot podría cambiar en algo el resultado. Además, queríamos tener
    cubiertas todas las posibilidades, por lo que dejamos que el Bot fuera el
    primero en realizar una jugada.
    \item \textit{Resultados}\\
    Con una muestra de 200 partidas generadas, obtuvimos los siguientes resultados (Fig. ~\ref{fig:botvsagent}.):
    \begin{itemize}
      \item \textit{Victorias de Player 1}: 2
      \item \textit{Victorias de Player 2}: 198
      \item \textit{Empates}: 0
    \end{itemize}
    Dejar al bot jugar primero no significó mucha diferencia.

    \begin{figure}
      \centering
      \includegraphics[width=\columnwidth]{BOTvsAGENT_MCTS.png}
      \caption{Resultados de Bot contra Agente MCTS.}
      \label{fig:botvsagent}
    \end{figure}
...
\item \textit{Agente MCTS vs Agente MCTS}:
  \begin{enumerate}
    ...
    \item \textit{Resultados}\\
    Con una muestra de 200 partidas generadas, obtuvimos los siguientes resultados:
    \begin{itemize}
...
que teníamos que realizar y dividimos el trabajo en varios hitos a realizar:
\begin{itemize}
...
maneja self, ...). Por último, el entrenamiento de la red neuronal nos permitió aprender
una aproximación de cómo se puede utilizar un modelo de aprendizaje supervisado en un
proyecto real y también que la dificultad de obtener buenas predicciones no es tan
fácil a cómo se ha visto en clase (con datos preparados de antemano y no generados
por nosotros).
...
y cómo algo que a priori puede estar algo oculto (ventaja para el jugador hace el segundo
